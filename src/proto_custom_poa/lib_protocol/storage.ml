...
module ValidatorSet :
  Simple_single_data_storage with type value = Account_repr.t list =
  Make_single_data_storage (Registered) (Raw_context)
    (struct
      let name = ["validator_set"]
    end)
    (struct
      type t = Account_repr.t list

      let encoding = Data_encoding.list Account_repr.encoding
    end)
...
module Account = struct
  ...
  module Votes =
    Indexed_context.Make_map
      ...
      (struct
        type t = Vote_repr.stored_action list

        let encoding = Data_encoding.list Vote_repr.stored_action_encoding
      end)
end
