...
let mine_worker (cctxt : Protocol_client_context.full) state account
  ...
  let rec worker_loop to_mine () =
    if Int32.equal 0l to_mine then return_unit
    else
      ...
      cctxt#message
        "Current Header %s"
        (Block_header_repr.to_string_json header)
      >>= fun () ->
      Client_proto_commands.get_next_target
        cctxt
        header.shell.level
        header.shell.timestamp
      >>=? fun current_target ->
      cctxt#message
        "Got target as bytes %s"
